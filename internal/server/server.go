...
func (s *Server) handleAbookTestLogin(w http.ResponseWriter, r *http.Request) {
	...
	if cfg.AbookEmail == "" || cfg.AbookPassword == "" {
		writeJSON(w, http.StatusBadRequest, map[string]any{"error": "abook_email and abook_password must be set before testing login"})
		return
	}
	...
	defer cancel()
	if err := client.Login(ctx, cfg.AbookEmail, cfg.AbookPassword); err != nil {
		writeJSON(w, http.StatusBadGateway, map[string]any{"ok": false, "error": err.Error()})
		return
	}
	cfg.AbookCookie = client.CookieHeader()
	if err := s.deps.Store.UpdateAppConfig(r.Context(), cfg); err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]any{"error": "saved cookie failed: " + err.Error()})
		return
	}
	writeJSON(w, http.StatusOK, map[string]any{
		"ok":             true,
		"cookieLength":   len(cfg.AbookCookie),
		"loggedInMarker": "session upgraded; cookie persisted",
	})
}
...
<article class="panel"><div class="panel-head"><div><h2>Plugin config</h2><p class="muted">AudiobookBay mirror, download mode, qBittorrent, embedded torrent, tracker, and search settings live in this plugin database.</p></div><span id="config-state" class="badge">Loading</span></div><form id="config-form" class="config-grid"><label>Base URL<input id="cfg-base-url" placeholder="https://audiobookbay.lu"></label><label>Download mode<select id="cfg-download-mode"><option value="scrape_only">scrape_only</option><option value="qbittorrent">qbittorrent</option><option value="embedded">embedded (torrent)</option><option value="embedded_nzbget">embedded_nzbget (bundled NZBGet)</option></select></label><label>qBittorrent URL<input id="cfg-qbit-url" placeholder="http://qbittorrent:8080"></label><label>qBittorrent username<input id="cfg-qbit-user"></label><label>qBittorrent password<input id="cfg-qbit-pass" type="password" placeholder="Leave blank to keep current password"></label><label>qBittorrent category<input id="cfg-qbit-category"></label><label>qBittorrent save path<input id="cfg-qbit-save-path"></label><label>Embedded download dir<input id="cfg-embedded-dir"></label><label>Embedded listen port<input id="cfg-embedded-port" type="number" min="0" max="65535"></label><label>Embedded max concurrent torrents<input id="cfg-embedded-max" type="number" min="0" max="64" placeholder="0 = default (8)"></label><label>Usenet host<input id="cfg-usenet-host" placeholder="news.example.com"></label><label>Usenet port<input id="cfg-usenet-port" type="number" min="0" max="65535" placeholder="563"></label><label>Usenet SSL<input id="cfg-usenet-ssl" type="checkbox"></label><label>Usenet username<input id="cfg-usenet-user" autocomplete="off"></label><label>Usenet password<input id="cfg-usenet-pass" type="password" autocomplete="off" placeholder="Leave blank to keep current"></label><label>Usenet connections<input id="cfg-usenet-conn" type="number" min="0" max="64" placeholder="0 = default (8)"></label><label>abook base URL<input id="cfg-abook-base" placeholder="https://abook.link/book"></label><label>abook email<input id="cfg-abook-email" type="email" autocomplete="off"></label><label>abook password<input id="cfg-abook-pass" type="password" autocomplete="off" placeholder="Leave blank to keep current"></label><label>NZBGet URL<input id="cfg-nzbget-url" placeholder="http://nzbget.lan:6789"></label><label>NZBGet username<input id="cfg-nzbget-user" autocomplete="off"></label><label>NZBGet password<input id="cfg-nzbget-pass" type="password" autocomplete="off" placeholder="Leave blank to keep current"></label><label>NZBGet category<input id="cfg-nzbget-cat" placeholder="audiobooks"></label><label>Search limit<input id="cfg-search-limit" type="number" min="1" max="100"></label><label class="span-all">Fallback trackers JSON<textarea id="cfg-trackers" rows="5" placeholder='["udp://tracker.opentrackr.org:1337/announce"]'></textarea></label><button type="submit">Save config</button></form><div class="row" style="grid-template-columns:auto auto minmax(0,1fr);gap:8px;margin-top:10px"><button id="abook-test" type="button">Test abook login</button><button id="nzbget-test" type="button">Test NZBGet</button><span id="abook-nzbget-status" class="muted" style="font-size:12px;align-self:center">—</span></div><pre id="config-output" class="output">Loading config...</pre></article>
</section>
...
document.getElementById("config-form").addEventListener("submit",async e=>{e.preventDefault();configState.textContent="Saving";try{const body={base_url:document.getElementById("cfg-base-url").value.trim(),download_mode:document.getElementById("cfg-download-mode").value.trim()||"scrape_only",qbittorrent_url:document.getElementById("cfg-qbit-url").value.trim(),qbittorrent_username:document.getElementById("cfg-qbit-user").value,qbittorrent_password:document.getElementById("cfg-qbit-pass").value,qbittorrent_category:document.getElementById("cfg-qbit-category").value,qbittorrent_save_path:document.getElementById("cfg-qbit-save-path").value,embedded_download_dir:document.getElementById("cfg-embedded-dir").value,embedded_listen_port:Number(document.getElementById("cfg-embedded-port").value||0),embedded_max_concurrent:Number(document.getElementById("cfg-embedded-max").value||0),usenet_host:document.getElementById("cfg-usenet-host").value.trim(),usenet_port:Number(document.getElementById("cfg-usenet-port").value||0),usenet_ssl:document.getElementById("cfg-usenet-ssl").checked,usenet_username:document.getElementById("cfg-usenet-user").value,usenet_password:document.getElementById("cfg-usenet-pass").value,usenet_connections:Number(document.getElementById("cfg-usenet-conn").value||0),abook_base_url:document.getElementById("cfg-abook-base").value.trim(),abook_email:document.getElementById("cfg-abook-email").value.trim(),abook_password:document.getElementById("cfg-abook-pass").value,nzbget_url:document.getElementById("cfg-nzbget-url").value.trim(),nzbget_username:document.getElementById("cfg-nzbget-user").value,nzbget_password:document.getElementById("cfg-nzbget-pass").value,nzbget_category:document.getElementById("cfg-nzbget-cat").value.trim(),search_limit:Number(document.getElementById("cfg-search-limit").value||10),trackers:JSON.parse(document.getElementById("cfg-trackers").value||"[]")};const r=await fetch("./api/v1/admin/config",{method:"PATCH",headers:{...headers(),"Content-Type":"application/json"},body:JSON.stringify(body)});const d=await r.json();if(!r.ok)throw new Error(d.error||r.statusText);document.getElementById("cfg-qbit-pass").value="";document.getElementById("cfg-abook-pass").value="";document.getElementById("cfg-nzbget-pass").value="";document.getElementById("cfg-usenet-pass").value="";configState.textContent="Saved";configOutput.textContent=JSON.stringify(d,null,2);await loadConfig()}catch(err){configState.textContent="Error";configOutput.textContent=String(err)}})
...
async function runAbookTest(){const status=document.getElementById("abook-nzbget-status");const btn=document.getElementById("abook-test");btn.disabled=true;status.textContent="Testing abook login…";status.className="muted";try{const r=await fetch("./api/v1/admin/abook/test-login",{method:"POST",headers:headers()});const d=await r.json();if(!r.ok||d.ok===false)throw new Error(d.error||r.statusText);status.textContent="abook login OK · cookie persisted ("+(d.cookieLength||0)+" bytes)";status.className="ok";}catch(e){status.textContent="abook login failed: "+(e.message||e);status.className="bad";}finally{btn.disabled=false;}}
async function runNZBGetTest(){const status=document.getElementById("abook-nzbget-status");const btn=document.getElementById("nzbget-test");btn.disabled=true;status.textContent="Testing NZBGet…";status.className="muted";try{const r=await fetch("./api/v1/admin/nzbget/test",{method:"POST",headers:headers()});const d=await r.json();if(!r.ok||d.ok===false)throw new Error(d.error||r.statusText);status.textContent="NZBGet OK · v"+(d.version||"?");status.className="ok";}catch(e){status.textContent="NZBGet failed: "+(e.message||e);status.className="bad";}finally{btn.disabled=false;}}
